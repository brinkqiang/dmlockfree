...
TEST(KFifoMultiThreaded, dmkfifo) { // 测试套件和名称已修改
...
#include "dmatomic_kfifo.h"

TEST(KFifoMultiThreaded, dmatomic_kfifo) { // 测试套件和名称已修改

    // 使用 fmt::print，和您的示例一致
    fmt::print("Starting test for AtomicKFifo with {} elements (values 1 to {})...\n", kNumElements, kNumElements - 1);

    AtomicKFifo kfifo(kKfifoBufferByteCapacity); // 初始化 KFifo
